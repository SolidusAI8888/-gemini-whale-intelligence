...
export function CandlestickChart({ series, markers, onMarkerHover }: { series: PricePoint[]; markers: Marker[]; onMarkerHover?: (id: string | null) => void }) {
  ...
  const height = 460;
  const pad = { left: 14, right: 72, top: 28, bottom: 34 };
  useEffect(() => {
  ...
  const model = useMemo(() => {
    ...
    const x = (index: number) => pad.left + (index + 0.5) * plotWidth / Math.max(1, series.length);
    const y = (value: number) => pad.top + (max - value) / Math.max(0.0001, max - min) * plotHeight;
  ...
  return <div className="candle-host" ref={host} onMouseLeave={() => { setHover(null); onMarkerHover?.(null); }}>
    <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`自 2025 年 1 月 1 日起的 ${series.length} 根日线蜡烛及巨鲸行动标记`}>
      {yTicks.map((value) => <g key={value}><line x1={pad.left} x2={width - pad.right} y1={model.y(value)} y2={model.y(value)} className="candle-grid" /><text x={width - pad.right + 9} y={model.y(value) + 4} className="candle-axis">${value.toLocaleString('en-US', { maximumFractionDigits: value < 10 ? 2 : 0 })}</text></g>)}
