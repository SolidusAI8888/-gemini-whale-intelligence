...
export function WhalePool() {
  ...
  return <main className="whale-pool-page">
    <header className="analysis-header"><nav className="analysis-nav"><a href="/"><ArrowLeft size={17} />标的工作台</a><a href="/analysis">集中度分析</a></nav><div className="terminal-brand"><span><Activity size={18} /></span><div><strong>WHALE INTELLIGENCE</strong><small>巨鲸池</small></div></div><span className="analysis-live"><i />{data.whales.length} 个主体</span></header>
    ...
    <section className="whale-groups">{categories.map(({ name, icon: Icon, note }) => { const whales = filtered.filter((whale) => whale.category === name).sort((a, b) => name === '行政部门' && a.name === 'Donald J. Trump' ? -1 : name === '行政部门' && b.name === 'Donald J. Trump' ? 1 : b.weight - a.weight || Number(b.trackingStatus === 'disclosed') - Number(a.trackingStatus === 'disclosed') || a.name.localeCompare(b.name)); const disclosed = whales.filter((whale) => whale.trackingStatus === 'disclosed').length; return <article className="whale-group" key={name}><header><Icon size={20} /><div><h2>{name}</h2><p>{note}</p></div><b>{whales.length}<small>{disclosed} 有披露</small></b></header><div>{whales.length ? whales.map((whale) => { const hasHoldings = whale.holdings.length > 0; const countLabel = hasHoldings ? '持仓' : whale.filingCount > 0 ? '文件' : whale.trackingStatus === 'disclosed' ? '行动' : '状态'; const countValue = hasHoldings ? whale.holdings.length : whale.filingCount > 0 ? whale.filingCount : whale.trackingStatus === 'disclosed' ? whale.eventCount : '无有效披露'; return <details className="whale-card" key={`${whale.category}-${whale.name}`}><summary><span className="whale-avatar">{whale.name.slice(0, 1).toUpperCase()}</span><span><strong>{whale.name}</strong><small>{whale.role || whale.source}</small></span><span className="whale-weight"><small>权重</small><b>{whale.weight.toFixed(2)}</b></span><span className="whale-count"><small>{countLabel}</small><b>{countValue}</b></span><i>⌄</i></summary><div className="whale-holdings"><h3>正式披露记录</h3>{whale.holdings.length ? whale.holdings.map((holding) => { const ticker = holding.ticker === 'GOOGL' ? 'GOOG' : holding.ticker; const content = <><strong>{holding.ticker}</strong><span>{holding.amount}</span><small>{holding.source} · {holding.publishedAt || holding.occurredAt}</small></>; return coreTickers.has(ticker) ? <a href={`/assets/${ticker}`} key={`${whale.name}-${holding.id}`}>{content}</a> : <div className="whale-holding-row" key={`${whale.name}-${holding.id}`}>{content}</div>; }) : <p>{whale.filingCount > 0 ? '官方 OGE 目录已登记该披露，但文件尚未开放下载或未解析出通过质量门的股票持仓；不会把文件存在误报成持仓。' : whale.trackingStatus === 'disclosed' ? '已有可验证行动，但尚无可验证的当前持仓快照。' : '已纳入跟踪范围；目前没有通过数据质量门的交易或持仓披露，因此不会进入图表或评分。'}</p>}{whale.sourceUrl && <a className="whale-source" href={whale.sourceUrl} target="_blank" rel="noreferrer">查看正式披露</a>}</div></details>; }) : <div className="whale-group-empty">当前筛选下暂无主体。</div>}</div></article>; })}</section>
