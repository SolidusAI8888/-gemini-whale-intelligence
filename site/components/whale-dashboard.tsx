...
import { Activity, BarChart3, CalendarClock, ExternalLink, Search, ShieldCheck, Users } from 'lucide-react';
...
export function WhaleDashboard({ initialTicker = 'UBER' }: { initialTicker?: string }) {
  ...
  const actionRanks = [...data.concentration].sort((a, b) => b.score - a.score);
  const holdingRanks = [...data.holdingConcentration].sort((a, b) => b.score - a.score);
  const actionRank = actionRanks.findIndex((row) => row.ticker === concentrationTicker);
  const holdingRank = holdingRanks.findIndex((row) => row.ticker === concentrationTicker);
  const actionRow = actionRank >= 0 ? actionRanks[actionRank] : null;
  ...
  return <main className="terminal-shell">
    ...
    <section className="terminal-workspace">
      ...
      <aside className="terminal-right">
        ...
        <div className="asset-rank-strip"><div><small>行动集中度</small><strong>{actionRank >= 0 ? `#${actionRank + 1}` : '—'}</strong><span>{actionRow ? `${actionRow.score} 分 / ${actionRanks.length} 标的` : '暂无有效行动'}</span></div><div><small>持仓集中度</small><strong>{holdingRank >= 0 ? `#${holdingRank + 1}` : '—'}</strong><span>{holdingRow ? `${holdingRow.score} 分 / ${holdingRanks.length} 标的` : '暂无持仓快照'}</span></div></div>
        ...
        <div className="right-heading"><div><strong>{asset.ticker} 巨鲸持仓</strong><small>最新可见正式披露快照，不重复展示图中的交易流水</small></div><span>{visibleHoldings.length} 位</span></div>
        <div className="right-scroll">{visibleHoldings.length ? visibleHoldings.map((event) => <HoldingCard event={event} key={`holding-${event.id}`} />) : <div className="right-empty"><ShieldCheck size={24} /><strong>{selectedActor ? '该巨鲸暂无当前持仓快照' : '暂无当前持仓快照'}</strong><p>交易点仍可在价格图上按所选巨鲸筛选。</p></div>}</div>
      </aside>
    </section>
  </main>;
}
