...
import { Activity, ArrowLeft, Building2, ExternalLink, Network, Search, ShieldCheck, Users } from 'lucide-react';
...
export function ConcentrationAnalysis() {
  ...
  const [increaseQuery, setIncreaseQuery] = useState('');
  const [decreaseQuery, setDecreaseQuery] = useState('');
  ...
  const filterTicker = <T extends { ticker: string }>(rows: T[], query: string) => { const value = query.trim().toUpperCase(); return value ? rows.filter((row) => row.ticker.includes(value)) : rows; };
  const visibleIncrease = filterTicker(increase, increaseQuery);
  const visibleDecrease = filterTicker(decrease, decreaseQuery);
  ...
  return <main className="analysis-page">
    ...
    <section className="analysis-grid three-panels">
      <article className="analysis-panel"><header><div><p>INCREASE CONCENTRATION</p><h2>新建仓 / 加仓</h2></div><span>显示 {visibleIncrease.length} / {increase.length}</span></header><label className="analysis-search"><Search size={15} /><input aria-label="搜索新建仓或加仓股票" value={increaseQuery} onChange={(event) => setIncreaseQuery(event.target.value)} placeholder="输入股票代码，如 NVDA" /></label><div className="analysis-table-head"><span>排名 / 标的</span><span>主体</span><span>披露来源</span><span>买入金额</span><span>评分</span></div>{visibleIncrease.map((row) => { const index = increase.indexOf(row); return <button className="analysis-row" onClick={() => openDetail({ ticker: row.ticker, mode: 'increase' })} key={row.ticker}><span className="analysis-rank">{String(index + 1).padStart(2, '0')}<strong>{row.ticker}</strong></span><span>{row.increaseActors}</span><SourceList names={row.increaseSources} /><span>{formatMoney(row.increaseAmountUsd)}</span><span className="analysis-score"><b>{row.increaseScore}</b><i><em style={{ width: `${row.increaseScore}%` }} /></i></span></button>; })}</article>
      <article className="analysis-panel"><header><div><p>DECREASE CONCENTRATION</p><h2>减仓 / 清仓</h2></div><span>显示 {visibleDecrease.length} / {decrease.length}</span></header><label className="analysis-search"><Search size={15} /><input aria-label="搜索减仓或清仓股票" value={decreaseQuery} onChange={(event) => setDecreaseQuery(event.target.value)} placeholder="输入股票代码，如 UBER" /></label><div className="analysis-table-head"><span>排名 / 标的</span><span>主体</span><span>披露来源</span><span>卖出金额</span><span>评分</span></div>{visibleDecrease.map((row) => { const index = decrease.indexOf(row); return <button className="analysis-row" onClick={() => openDetail({ ticker: row.ticker, mode: 'decrease' })} key={row.ticker}><span className="analysis-rank">{String(index + 1).padStart(2, '0')}<strong>{row.ticker}</strong></span><span>{row.decreaseActors}</span><SourceList names={row.decreaseSources} /><span>{formatMoney(row.decreaseAmountUsd)}</span><span className="analysis-score"><b>{row.decreaseScore}</b><i><em className="negative-bar" style={{ width: `${row.decreaseScore}%` }} /></i></span></button>; })}</article>
      <article className="analysis-panel"><header><div><p>HOLDING CONCENTRATION</p><h2>当前披露持仓</h2></div><span>显示 {visibleHoldings.length} / {holdings.length}</span></header><label className="analysis-search"><Search size={15} /><input aria-label="搜索当前披露持仓股票" value={holdingQuery} onChange={(event) => setHoldingQuery(event.target.value)} placeholder="输入股票代码，如 MELI" /></label><div className="analysis-table-head holding"><span>排名 / 标的</span><span>持有人</span><span>披露来源</span><span>申报价值</span><span>评分</span></div>{visibleHoldings.map((row) => { const index = holdings.indexOf(row); return <button className="analysis-row holding" onClick={() => openDetail({ ticker: row.ticker, mode: 'holding' })} key={row.ticker}><span className="analysis-rank">{String(index + 1).padStart(2, '0')}<strong>{row.ticker}</strong></span><span>{row.holderCount}</span><SourceList names={row.sourceNames} /><span>{formatMoney(row.valueUsd)}</span><span className="analysis-score"><b>{row.score}</b><i><em style={{ width: `${row.score}%` }} /></i></span></button>; })}</article>
    </section>
